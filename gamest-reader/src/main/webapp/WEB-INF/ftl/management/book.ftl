...
<script>
    ...
    table.render({
        ...
        , cols: [[ //表头
            ...
            /*type:'space'表示"空"列,即没有数据,但不代表其没有内容,将该列作为操作列
            * templet用于对数据进行重新渲染来生成我们需要的html(由后面的函数构成);function(d)中的d表示表格中每一行的
            * 数据用d来指代*/
            , {
                type: 'space', title: '操作', width: '200', templet: function (d) {
                    //拼接字符串,为每一行表格数据生成"修改"与"删除"按钮,并附加data-id自定义属性代表图书编号
